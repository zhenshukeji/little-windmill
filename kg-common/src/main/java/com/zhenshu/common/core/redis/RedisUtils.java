package com.zhenshu.common.core.redis;
...
public class RedisUtils {
     ...
     * @return
     */
    public Set<TypedTuple<String>> reverseRangeWithScores(String key, long start,
                                                          long end) {
     ...
     * @param count    数量
     */
    public Set<String> zRangeByLex(String cacheKey, long min, long max, int offset, int count) {
        RedisZSetCommands.Limit limit = new RedisZSetCommands.Limit().count(count).offset(offset);
        RedisZSetCommands.Range range = new RedisZSetCommands.Range().lt(max).gt(min);
        return stringRedisTemplate.opsForZSet().rangeByLex(cacheKey, range, limit);
    }
}
